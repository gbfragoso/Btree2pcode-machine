...
string postOrder(string* btree, int node, string expr);
...
int main (int argc, char **argv){
    ...
    if (argc == 3){
    	...
    	if(file != NULL){
		    ...
		    Tinstruction * instructions = toInstruction(expr);
...
string postOrder(string* btree, int node, string expr){
    if(node >= treeSize || strcmp(btree[node], "0") == 0){
    	return NULL;
    }else{
    	postOrder(btree, 2*node+1, expr);
    	postOrder(btree, 2*node+2, expr);
